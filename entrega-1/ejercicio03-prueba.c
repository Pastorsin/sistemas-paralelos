...
#define ORDENXCOLINV 2
#define ORDENXFILASINV 3
...
double getValor(double *matriz,int fila,int columna,int orden){
  ...
  } else if (orden == ORDENXCOLUMNAS) {
    return(matriz[fila+columna*N]);

  } else {
    return(matriz[(N*N-1) - ((N-1-columna)*N+fila)]);
...
void setValor(double *matriz,int fila,int columna,int orden,double valor){
 if(orden==ORDENXFILAS){
  ...
  matriz[(N*N-1) - ((N-1-columna)*N+fila)]=valor;
 }
}
...
void init_matrices(double *A, double *B, double *C){
  ...
  setValor(C, 0, 0, ORDENXCOLINV, 1);
  setValor(C, 0, 1, ORDENXCOLINV, 2);
  setValor(C, 1, 0, ORDENXCOLINV, 3);
  setValor(C, 1, 1, ORDENXCOLINV, 1);
}
 ...
 //Realiza la multiplicacion
  ...
  for(i=0;i<N;i++){
    ...
    for (int r = 0; r < N; r++){
      ...
      for (int c = N-1; c >= 0; c--){
        // D[i, r] += AB[i, c] * C[c, r];
        suma += getValor(AB, i, c, ORDENXFILAS) * getValor(C, c, r, ORDENXCOLINV);
      }

      setValor(D, i, r, ORDENXFILAS, suma);
    }
