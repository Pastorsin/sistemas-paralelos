...
int main(int argc, char* argv[]) {
	...
	#pragma omp parallel
	{
		...
		#pragma omp for private(i,j,k,index) reduction(+:totalA,totalB) reduction(max:maxA,maxB) reduction(min:minA,minB) nowait
		...
		#pragma omp for private(i,j,k,index) reduction(+:totalC) reduction(max:maxC) reduction(min:minC)
		for (i = 0; i < stripSize; i++) {
			for (j = 0; j < N; j++) {
				double suma_parcial = 0;
